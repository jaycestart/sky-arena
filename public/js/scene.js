...
export class Scene {
  ...
  init() {
    ...
    this.far = this.caps.depthBits >= 24 ? 90000 : 40000;
    this.near = 4.0;   // 콕핏은 자기 기체를 건너뛰고, 3인칭은 34m 뒤다
  ...
  camera(me) {
    ...
    const back = (23 + clamp((spd - 400) / 1000, 0, 1) * 7) * ms;
    const cu = quat.up(q);
    return { q, eye: v3.add(me.pos, v3.add(v3.mul(cf, -back), v3.mul(cu, 6.2 * ms))) };
  }
