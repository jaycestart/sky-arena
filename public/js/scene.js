...
// **지형에 스쳐 지나갈 것이 없다**는 것이다. terrainH 의 최단 파장이 662m 라
// 이 세계에는 662m 보다 작은 지오메트리가 원리적으로 없고(유도는 gfx/terrain.js
// 머리 주석), 순항 고도 3000m 에서는 그 662m 짜리 언덕조차 화면에서 거의 안
// 움직인다. 속도감(= 시선 각속도)은 **가까운 작은 것**에서만 나온다.
// (여기 885m 로 적혀 있던 것은 최고 옥타브의 x 축 성분만 본 값이다 —
//  최단은 대각선 방향 661.9m 다. 2026-08-15 지형실 정정. m3d.js:164 는 아직
//  885m 인데 그쪽은 프로그래밍실 소관이라 남겨 두었다.)
//
...
const PAINT = {
  // ── 노출 보정을 되돌렸다 (2026-08-15) ─────────────────────────────
  // 2026-08-14(15a5ea3)에 이 표를 1.65배 어둡게 낮췄다. 이유는 '실물 반사율을
  // 넣으면 ACES 어깨에서 포화한다' 였는데, **포화의 원인은 도장이 아니라
  // 노출이었다.** 그때는 화면을 잴 수 없어(브라우저 패널이 숨으면 rAF 가 멈춰
  // 캔버스가 전부 0 으로 읽혔다) 조명 경로를 읽어 도장 쪽을 보정했다.
  //
  // 오늘 노출을 0.72EV(=1.65배) 내렸으므로(`BASE_EXPOSURE` 주석) 그 보정은
  // 이중 보정이 된다. 15a5ea3 **이전 값을 그대로 되돌린다.** 두 변경은 기체에
  // 대해 정확히 상쇄하고, 남는 것은 지형·하늘이 내려간 몫뿐이다.
  //
  // 그래서 이 값은 이제 다시 **실물 도장의 반사율**이다. 되돌리기 전 값이
  // 궁금하면 15a5ea3 을 보라 — 다시 낮추지 마라. 화면이 뜨면 노출을 봐라.
  //
  // 조절은 여기 세 줄이 아니라 `BASE_EXPOSURE` 로 한다. 위/아래 비율은
  // 유지해야 한다 — 그게 카운터셰이딩(배 밑만 밝은 실제 도장 방식)의 근거다.
  // F-16 형 — 2톤 고스트 그레이(상 FS36270 / 하 FS36375)
  falcon: { top: new Float32Array([0.179, 0.205, 0.223]),
            under: new Float32Array([0.478, 0.506, 0.522]), gold: 1 },
  // F-15 형 — Mod Eagle 저대비 2톤. 위아래 차가 거의 없는 것이 특징이다.
  eagle: { top: new Float32Array([0.147, 0.163, 0.171]),
           under: new Float32Array([0.229, 0.250, 0.261]), gold: 0 },
  // Su-27 형 — 청회색. 유일하게 색상(hue)까지 다르다.
  flanker: { top: new Float32Array([0.095, 0.147, 0.212]),
             under: new Float32Array([0.522, 0.568, 0.603]), gold: 0 },
};
...
// 기준 노출. 태양 조도 선형 1.0 = 100,000 lux 규약 위에서 잡는다. 자동 노출은
// 이 값 주위를 오갈 뿐이다(`FS_POST` 의 `clamp(0.026/평균, 0.40, 2.50)`).
//
// ── EV -3.2 → -2.48 (2026-08-15) ──────────────────────────────────────
// 예전 앵커는 '하늘이 중간 밝기로 오도록' 이었다. 그런데 하늘을 중간에 놓으면
// **해를 받는 면은 전부 그 위로 올라간다.** 실측(실제 서버·실제 판에 헤드리스로
// 붙어 HDR 리졸브 타깃을 직접 읽음. 1066×479, Intel Iris Xe / ANGLE D3D11,
// 24표본의 중앙값):
//
//   프레임 전체 중앙값 158 · 지형(화면 아래 1/4) 181 · 하늘(위 22%) 100
//   중간회색은 118 이다. 즉 지형이 2/3EV 넘게 들려 있었다.
//
// **자동 노출은 범인이 아니다** — 게인 중앙값 1.138 이고 하한 0.40 에 붙은
// 표본이 0/24 다(오히려 밝히는 쪽으로 돌고 있었다). 하한에 붙는 것은
// SwiftShader 로 재면 나오는 현상인데, 그 경로는 기체 반사가 선형 1.98 까지
// 튀어 실제 GPU 와 그림 자체가 다르다. 노출은 실제 GPU 로만 재라.
//
// 0.72EV(=1.65배) 내렸다. 이 배수는 임의가 아니다 — 2026-08-14(15a5ea3)에
// **바로 이 포화를 보정하려고** 기체 도장을 1.65배 어둡게 낮췄다. 노출을 그만큼
// 내리고 도장을 원래 값으로 되돌리면 기체 밝기는 그대로고 지형·하늘만 내려간다.
// 우연히도 같은 배수에서 프레임 중앙값이 158 → 121 로 중간회색과 만난다.
// 두 갈래로 따로 나온 숫자가 겹쳤다.
const BASE_EXPOSURE = 1.0 / (1.2 * Math.pow(2, -2.48));
...
// 적응형 강등 사슬 — 내릴 땐 즉시, 올릴 땐 2초 안정 후(`adapt` 의 `_degT > 2`).
//
// 예전 사슬은 맨 끝까지 내려가도 후처리·물·파티클이 켜진 채였다. 그래서
// 정말 느린 기기에서는 아무리 강등해도 60fps 에 닿지 못했다. 끝을 '옛
// 렌더러와 같은 수준'까지 열어 둬야 어떤 기기에서든 결국 부드러워진다.
// 아래 표의 실제 순서다: 블룸 해상도 → 구름 → 지형 디테일 → 픽셀 수 → 물
// → 파티클 → 후처리 전체. (여기 적혀 있던 "픽셀 수 → 구름 → 블룸 → 물 →
// 디테일"은 해상도부터 깎던 **옛 표**를 설명한 채 남아 있던 것이다. 바로
// 다음 문단이 "해상도는 마지막까지 지킨다"고 정반대로 적고 있었다.)
...
export class Scene {
  ...
  /** 프레임 시간 80퍼센타일로 품질을 히스테리시스 조정한다.
   *
  ...
  adapt(dt) {
    ...
    // 창의 위에서 다섯 번째(p95)로 판정하면 **가끔 튀는 빠른 기기**가 느린
    // 기기로 읽힌다. 90프레임 창에서 p95 는 위에서 5번째라 창의 5.6% 만
    // 느려도 문턱을 넘는다. 실측(62fps 에 40ms 프레임을 3%만 섞어 30초):
    // 강등 7회, 맨 아래 칸까지 내려가 **구름이 전체 시간의 73% 동안 꺼져
    // 있었다.** 8% 면 93%, 12% 면 96%다. 정작 그 기기는 프레임의 97%를
    // 62fps 로 그리고 있다. 사장님이 "하늘에 구름이 없다"고 한 자리다.
    //
    // p80 은 '창의 5분의 1 이상이 느릴 때만' 느린 기기로 본다. 아홉 가지
    // 시나리오를 p50·p70·p80·p90·p95·평균으로 판정시켜 보면 p70 과 p80 만
    // 전부 맞힌다(p95 는 2개, p50·평균은 1개씩 틀린다). 둘 중 진짜 느린
    // 기기를 더 일찍 잡는 쪽을 골랐다.
    //   60fps X · 50fps X · 62fps+3%튐 X · +8%튐 X · +15%튐 X
    //   · 반반 16/40ms(평균 39fps) O · 26.5ms O · 33ms O · 50ms O
    //
    // **복구도 반드시 같은 통계량이어야 한다.** 강등만 p80 으로 바꾸고 복구를
    // p95 로 두면, 튀는 기기는 한 번 내려간 뒤 p95 가 영영 16.5 아래로 안
    // 내려와 다시는 못 올라온다 — 구름이 한 번 꺼지면 안 돌아오던 이유다.
    const ft = s[Math.floor(s.length * 0.80)];
    this._degT += dt;
    ...
    if (ft > 26.0 && this.deg < last) {
      // 얼마나 느린지에 비례해 건너뛴다 — 40ms(25fps)면 세 칸을 한 번에.
      //
      // 가운데 문턱이 26 이면 **바깥 if 와 같은 조건**이라 `: 1` 이 도달 불가다.
      // 실측(진짜 adapt 를 껍데기 this 에 call, 일정 프레임시간 400프레임):
      // 26.5·27·28·30·33·35·38·40ms 가 **전부 첫 강등에서 두 칸**을 뛰었다.
      // 즉 38fps~25fps 사이가 한 덩어리로 '많이 느림' 취급이었고, 내려가는
      // 길에서 DEGRADE[1](블룸 반해상도)에 **한 번도 못 앉는다** — 37fps 짜리
      // 기기도 첫 강등에서 곧장 구름이 꺼졌다. 위 표가 "눈에 덜 띄는 것부터
      // 끈다"고 정해 둔 순서가 첫 계단에서부터 무너져 있었다.
      //
      // 33ms(30fps)는 주석이 이미 적어 둔 두 문턱 26(38fps)·40(25fps)의
      // 가운데다 — 새 숫자를 지어내지 않는다. fps 로도 38 / 30 / 25 로 고르다.
      const jump = ft > 40 ? 3 : (ft > 33 ? 2 : 1);
    ...
    } else if (ft < 16.5 && this.deg > 0 && this._degT > 2) {
