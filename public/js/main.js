...
export const BUILD = '2026-08-15c';

// 화면 오른쪽 아래에 빌드 날짜를 띄우려고 월드에 넘긴다. 반드시 위
// 선언 **뒤**여야 한다 — const 는 선언 전에 쓰면 ReferenceError 로
// 모듈이 통째로 죽는다. 이 값이 화면에 없으면 '바뀐 게 맞냐'를 말로
// 주고받게 되는데, 오늘 코드는 올라갔는데 화면은 어제 것인 상태를
// 알아채는 데 한참 걸렸다.
world.build = BUILD;
...
function onEvent(ev) {
  if (ev.e === 'kill') {
    ...
    // `ev.k` 는 서버가 실어 보내는 무기 종류다(`game.py` `_launch`:
...
function loop(now) {
  ...
  if (world.me && world.cfg) {
    ...
    if (world.srv?.al) {
      ...
      // 때마다 높이가 튀어 접근 속도가 헛값이 된다.
      sfx.ground(world.srv.agl);
    } else {
