...
class Pool {
  ...
  step(dt) {
    ...
    for (let i = 0; i < this.n;) {
      const b = i * F;
      s[b + P_AGE] += dt;
      // **나이가 음수면 아직 안 태어났다** — 안 움직이고 build() 도 건너뛴다.
      // 지연에 슬롯을 따로 안 쓰는 이유는 P_ 오프셋을 한 칸도 안 밀기
      // 위해서다(맨 위 P_SW 주석과 같은 이유). 음수 나이는 어차피 쓰이지
      // 않던 구간이라 F 를 늘리지 않고 그대로 뜻을 얹을 수 있다.
      if (s[b + P_AGE] < 0) { i++; continue; }
...
export class Fx {
  ...
  _p(pool, x, y, z, vx, vy, vz, o) {
    ...
    // o.delay 초 뒤에 태어난다(Pool.step 주석). 수명은 태어난 뒤부터 센다.
    s[b + P_AGE] = o.delay ? -o.delay : 0;
    s[b + P_LIFE] = o.life;
  ...
  // ── 이펙트 ──────────────────────────────────────────────────────
  /**
   * 격추 폭발 — 백열 코어 · 충격파 링 · 끓는 화구 · 검은 연기 · 파편.
   *
   * **이 이펙트에는 시간 구조가 있다.** 예전에는 65개가 전부 한 프레임에
   * 태어났다. 검은 연기 22개가 화구와 **같은 순간에 같은 점에서** 시작해,
   * t=0 에 이미 불의 6할을 덮고 있었다(계측). 폭발이 '펑' 이 아니라 한 장의
   * 그림으로 읽히던 이유다. 이제 delay 로 섬광 → 화구 → 연기 순서를 만든다.
   * 연기의 첫 인스턴스는 0.117초에 나온다(검증).
   *
   * **연기가 이 이펙트 비용의 94.6% 다**(눈에서 60m, 누적 기준). 그래서 여기서
   * 태어나는 때를 흩는 것이 곧 성능이기도 하다 — 22개가 함께 커져 같은 순간에
   * 최대가 되던 것이 시간축으로 퍼진다. 개수는 그대로다.
   *
   * 늦게 나는 조각은 **자기 속도로 그때까지 흘러간 자리**에서 태어난다.
   * 지연 중에는 step() 이 안 움직이므로, 제자리에서 늦게 내면 이미 퍼진
   * 화구 한복판에 새 덩어리가 툭 나타난다.
   */
  explosion(x, y, z) {
    const R = Math.random;
    // ── 0.00s 백열 코어 ──────────────────────────────────────────────
    // 근접신관 공중폭발(airburst)에도 명중 스파크(spark)에도 흰 섬광이 있는데
    // **게임에서 가장 큰 사건인 격추 폭발에만 없었다.** 첫 프레임에 제일 밝은
    // 것이 주황색 T_FLAME 이라 '터졌다' 가 아니라 '불이 붙었다' 로 읽힌다.
    // 0.12초짜리라 연기 예산과 겹치지 않는다.
    this._p('add', x, y, z, 0, 0, 0, {
      life: 0.12, s0: 22, s1: 64, c0: [12.0, 10.6, 7.6], c1: [4.0, 1.8, 0.5], tile: T_GLOW,
    });
    // 충격파 링
    this._p('add', x, y, z, 0, 0, 0, {
    ...
    // ── 0.00~0.30s 끓는 화구 ─────────────────────────────────────────
    // 예전 16개는 전부 t=0 에 나고 수명이 0.5~1.0초였다. 스프라이트는 s0→s1
    // 로 선형으로 커지므로 **덩어리가 가장 큰 순간은 죽기 직전**이고, 그래서
    // 화구 면적의 최대가 t=0.57 이었다 — 터지고 반 박자 뒤에 제일 크다.
    // 태어나는 때를 0.30초에 걸쳐 흩고 수명을 0.34~0.68 로 줄이면 최대가
    // 0.43초로 당겨지면서(계측, 200m) 죽는 시각도 함께 흩어진다.
    //
    // (t=0.5 뒤에 가산 파티클 면적이 3.6배 급락하는 것은 **충격파 링**이
    // 수명 0.55 로 사라지는 것이지 화구가 아니다. 처음 그걸 화구 탓으로
    // 적었다가 종류별로 다시 재서 바로잡았다 — 링은 안 건드렸다.)
    for (let i = 0; i < 18; i++) {
      const a = R() * 6.283, e = (R() - 0.5) * 2;
      const sp = 16 + R() * 40;
      const vx = Math.cos(a) * sp, vy = e * sp * 0.7, vz = Math.sin(a) * sp;
      const d = i < 6 ? 0 : (i - 6) / 12 * 0.30;      // 앞 6개는 즉발
      this._p('add', x + vx * d * 0.7, y + vy * d * 0.7, z + vz * d * 0.7, vx, vy, vz, {
        delay: d,
        life: 0.34 + R() * 0.34, s0: 14 + R() * 16, s1: 40,
        rot: R() * 6.283, rotV: (R() - 0.5) * 2,
        c0: [7.0, 4.2, 1.4], c1: [1.4, 0.24, 0.05], tile: T_FLAME, drag: 2.6,
      });
    }
    // ── 0.10~0.95s 검은 연기 ─────────────────────────────────────────
    // 불이 한 번 부풀고 난 뒤부터 오른다. 개수·수명·최대 크기는 예전 그대로다
    // — 바뀐 것은 **태어나는 때**뿐이고, 그것만으로 화면이 순서를 얻는다.
    for (let i = 0; i < 22; i++) {
      const a = R() * 6.283, sp = 6 + R() * 22;
      const vx = Math.cos(a) * sp, vy = 5 + R() * 12, vz = Math.sin(a) * sp;
      const d = 0.10 + (i / 22) * 0.85;
      this._p('alpha', x + vx * d * 0.7, y + vy * d * 0.7, z + vz * d * 0.7, vx, vy, vz, {
        delay: d,
        // **지연은 수명에서 뺀다.** 그냥 늦추면 죽는 시각까지 같이 밀려
        // 큰 덩어리가 동시에 살아 있는 창이 오히려 넓어진다 — 처음 이렇게
        // 짰다가 200m 최대 오버드로가 0.997 → 1.115 로 나빠졌다. 연기 기둥이
        // 걷히는 시각은 예전 그대로 두고 **나타나는 때만** 흩는 것이 맞다.
        life: 2.4 + R() * 2.2 - d, s0: 18, s1: 110,
        rot: R() * 6.283, rotV: (R() - 0.5) * 0.7,
        c0: [0.05, 0.045, 0.042], c1: [0.16, 0.155, 0.15], tile: T_SMOKE,
        drag: 0.9, grav: -0.4, a0: 0.72,
      });
    }
    // ── 0.00s 파편 ───────────────────────────────────────────────────
    // 즉발이다. 폭발에서 제일 먼저 눈에 띄는 것이 사방으로 튀는 조각이고,
    // 이것만은 늦게 나면 안 된다. 누적 비용의 0.05% 라 손대지 않았다.
    for (let i = 0; i < 26; i++) {
      const a = R() * 6.283, e = (R() - 0.5) * 2;
      const sp = 40 + R() * 130;
      this._p('add', x, y, z, Math.cos(a) * sp, e * sp, Math.sin(a) * sp, {
        life: 0.7 + R() * 0.9, s0: 2.4, s1: 0.6,
        c0: [9.0, 5.0, 1.6], c1: [2.0, 0.35, 0.08], tile: T_CHUNK,
        rot: R() * 6.283, rotV: (R() - 0.5) * 9,
        drag: 0.7, grav: -9.8,
      });
    }
  }
   ...
   *
   * 스파크는 **동그란 점이 아니라 속도 방향으로 늘인 줄**이다. 엔진에 이미
   * 스트레치 빌보드가 있는데(VS_FX 의 iStretch, 예광탄이 쓴다) 여기서만 안
   * 쓰고 있었다. 튀는 불똥은 카메라 노출 시간 동안 궤적을 남기므로 점으로
   * 그리면 '반짝이는 가루' 가 된다.
   *
   * 길이는 속도에 비례하므로(drag 3.2 로 감속) **줄이 저절로 짧아지며 꺼진다** —
   * 크기 곡선으로 흉내 낼 필요가 없다. 폭을 1.5 → 0.34 로 줄여 줄답게 만들면
   * 스프라이트 넓이는 오히려 준다(2.25 → 최대 1.3 m²).
   */
  spark(x, y, z, seed) {
    const n = 24;
    for (let i = 0; i < n; i++) {
      ...
      this._p('add', x, y, z, Math.cos(a) * sp, e * sp * 0.8, Math.sin(a) * sp, {
        life: 0.22 + Math.random() * 0.3, s0: 0.34, s1: 0.12,
        c0: [8.0, 5.5, 2.0], c1: [2.0, 0.5, 0.1], tile: T_SPARK,
        drag: 3.2, grav: -9.8, stretch: 0.055,
      });
    }
    for (let i = 0; i < 4; i++) {
  ...
  airburst(x, y, z, dir) {
    ...
    for (let i = 0; i < 18; i++) {
      ...
      // 명중 스파크와 같은 이유로 줄이다(spark() 주석). rot·rotV 는 뺐다 —
      // 스트레치가 걸리면 VS_FX 가 회전을 아예 안 읽는다(늘인 축이 기저를
      // 정한다). 두면 step() 이 매 프레임 적분만 하고 화면에는 안 나온다.
      this._p('add', x, y, z,
              e0[0] * ca + e1[0] * sa, e0[1] * ca + e1[1] * sa, e0[2] * ca + e1[2] * sa, {
        life: 0.28 + Math.random() * 0.3, s0: 0.40, s1: 0.14,
        c0: [8.0, 5.0, 1.8], c1: [1.8, 0.4, 0.1], tile: T_SPARK,
        drag: 1.6, grav: -9.8, stretch: 0.05,
      });
    }
  ...
  /**
   * 저공 흙먼지 — 지면을 스치고 지날 때 지상 항적에서 일어나는 모래.
   *
   * **기체 뒤가 아니라 앞에 놓는다.** 물리적으로 맞는 자리는 후류(뒤)지만,
   * 3인칭 카메라가 기체보다 17.5m 뒤에서 앞을 보므로 뒤에 놓으면 스폰과
   * 동시에 화면 밖으로 나간다 — 1.5초를 살아도 사용자는 한 프레임도 못 본다.
   * 스폰 지점은 호출자(scene.js)가 항적 앞쪽에서 잡아 넘긴다.
   *
   * 위로 솟는 성분을 작게 둔 이유: 크게 주면 먼지 기둥이 되어 저공에서
   * 시야를 가린다. 속도감을 얻으려고 조준을 잃으면 손해다 — 속도 블러를
   * 껐던 것과 같은 판단이다.
   *
   * @param dx,dz  기체 진행 방향의 수평 단위벡터(먼지가 끌려가는 쪽)
   * @param k      0..1 짙기(고도가 낮을수록 1)
   */
  groundDust(x, y, z, dx, dz, spd, k) {
    const j = () => (Math.random() - 0.5) * 2;
    // 끌려가는 속도는 기체 속도에 비례하되 상한을 둔다. 비례만 시키면
    // AB 1450m/s 에서 먼지가 총알처럼 날아가 '연기 예광탄'이 된다.
    const drag = Math.min(46, 10 + spd * 0.045);
    this._p('alpha', x, y + 1.2, z,
            dx * drag + j() * 7, 2.5 + Math.random() * 5, dz * drag + j() * 7, {
      life: 1.0 + Math.random() * 0.7, s0: 4.5, s1: 32 + Math.random() * 20,
      c0: [0.42, 0.35, 0.25], c1: [0.30, 0.27, 0.22], tile: T_SMOKE,
      drag: 1.1, grav: -0.9, a0: 0.34 * k,
      rot: Math.random() * 6.283, rotV: (Math.random() - 0.5) * 0.6,
    });
  }

  /**
   * 익단 와류 — 급선회에서 날개 끝 저압부에 생기는 흰 응결 소용돌이.
   *
   * 점으로 찍지 않고 **월드 고정 스트레치(sw)로 짧은 리본**을 만든다.
   * 점을 이어 붙이면 염주알이 되는 건 미사일 궤적에서 이미 겪었다
   * (missile() 주석). 리본이면 발생률을 절반으로 줄여도 끊겨 보이지 않아
   * 오히려 파티클이 덜 든다.
   *
   * @param bx,by,bz  리본 방향·길이(월드). 보통 -진행방향 × 길이.
   */
  vortex(x, y, z, bx, by, bz, k) {
    this._p('alpha', x, y, z, 0, 0, 0, {
      life: 0.42 + Math.random() * 0.28, s0: 0.9, s1: 5.2,
      c0: [0.95, 0.97, 1.0], c1: [0.82, 0.86, 0.94], tile: T_SMOKE,
      a0: 0.44 * k, rot: Math.random() * 6.283, sw: [bx, by, bz],
    });
  }
   ...
   * 지면·수면 착탄.
   *
   * **불은 이 함수의 몫이 아니다.** scene.js 는 지면 boom 에 explosion() 을
   * 먼저 부르고 그 위에 이것을 얹는다(scene.js:1782·1784). 그러니 여기서 낼
   * 것은 '땅(물)에 닿았다' 는 신호 하나뿐이다.
   *
   * 예전에는 그 신호가 없었다. 퍼프 12개가 **전부 t=0 에 같은 점에서** 나
   * 위로만 올라갔다 — 헤드리스 실측으로 21.4m 까지 오르고 한 개도 안
   * 떨어진다(grav −1.5 는 사실상 무중력이다). 위로 오르는 기둥은 공중
   * 폭발에도 있으므로, 검은 연기 기둥 옆에 갈색이 조금 섞인 것 말고는
   * 공중에서 터진 것과 구별되는 것이 하나도 없었다.
   *
   * **지면 접촉을 읽게 하는 것은 수직이 아니라 수평이다.** 폭풍이 땅을
   * 때리면 갈 데가 옆밖에 없어 먼지가 지면에 붙어 고리로 퍼진다(base
   * surge). 그래서 예산을 위쪽 기둥에서 떼어 옆으로 옮겼다.
   *
   * **알파 개수는 예전 그대로다 — 지면 12개 · 수면 13개.** 비용을 지배하는
   * 것은 큰 반투명 스프라이트이고, 늘리면 24인 방에서 알파 풀이 조용히
   * 마른다(Pool.spawn 의 −1, setCap 주석). 늘어난 것은 가산 풀의 탄도
   * 조각뿐이다 — 지면 16개 · 수면 20개.
   *
   * **다만 가산 풀도 공짜는 아니다.** 조각은 작고 짧은 스프라이트라 픽셀
   * 비용은 알파 퍼프에 비할 바가 아니지만, **풀 칸은 똑같이 한 칸씩 먹는다.**
   * scene.js 가 지면 boom 마다 explosion() 과 이것을 같이 부르므로
   * (scene.js:1782·1784) 한 발이 가산 풀에서 쓰는 몫이 46 → 62 개가 된다.
   * 실측으로 low 프리셋(가산 풀 200)에서 동시 지면 폭발이 **5발째가 아니라
   * 4발째에** 풀을 채운다. 그 뒤로는 spawn 이 −1 을 돌려 조각이 조용히
   * 사라진다 — 알파 쪽에서 실제로 겪었던 그 고장이다(setCap 주석).
   * 조각 수를 더 올리려면 이 수를 다시 재고 올려라.
   */
  impact(x, y, z, water) {
    if (water) this._splash(x, y, z);
    else this._dirt(x, y, z);
  }

  /** 흙 착탄 — 접지 먼지 고리 6 · 흙 기둥 6 · 탄도 흙줄기 16(가산). */
  _dirt(x, y, z) {
    const R = Math.random;
    // ── 0.00~0.10s 접지 먼지 고리 ────────────────────────────────────
    // 거의 수평으로만 나간다. 이 고리 하나가 '땅에 닿았다' 를 만든다.
    // drag 2.4 라 대략 v/drag ≈ 19m 까지 퍼지고 선다 — 화면 밖으로 달아나지
    // 않고 제자리에서 부푼다. 즉발 셋 · 나머지는 0.05·0.10 뒤에 난다.
    for (let i = 0; i < 6; i++) {
      const a = (i / 6) * 6.283 + R() * 0.9;
      const sp = 26 + R() * 20;
      const d = (i % 3) * 0.05;
      const vx = Math.cos(a) * sp, vy = 1.0 + R() * 2.0, vz = Math.sin(a) * sp;
      // 늦게 나는 것은 제 속도로 그때까지 흘러간 자리에서 태어난다
      // (explosion() 과 같은 규칙 — 제자리에 내면 이미 퍼진 고리 한복판에
      // 새 덩어리가 툭 나타난다). 지연은 수명에서 뺀다.
      this._p('alpha', x + vx * d * 0.7, y + 1.4 + vy * d, z + vz * d * 0.7, vx, vy, vz, {
        delay: d,
        life: 1.2 + R() * 0.5 - d, s0: 4, s1: 26,
        c0: [0.40, 0.34, 0.25], c1: [0.30, 0.28, 0.24], tile: T_SMOKE,
        drag: 2.4, grav: -0.5, a0: 0.52,
        rot: R() * 6.283, rotV: (R() - 0.5) * 0.5,
      });
    }
    // ── 0.05~0.35s 흙 기둥 ───────────────────────────────────────────
    // 예전 12개가 하던 일을 6개가 한다. 좁아진 대신(수평 3~10, 예전 4~20)
    // 기둥으로 서고, grav −5 라 끝에서 처져 되떨어지기 시작한다.
    for (let i = 0; i < 6; i++) {
      const a = R() * 6.283, sp = 3 + R() * 7;
      const vx = Math.cos(a) * sp, vy = 16 + R() * 18, vz = Math.sin(a) * sp;
      const d = 0.05 + (i / 6) * 0.30;
      this._p('alpha', x + vx * d, y + vy * d * 0.8, z + vz * d, vx, vy, vz, {
        delay: d,
        life: 1.3 + R() * 0.5 - d, s0: 4, s1: 24,
        c0: [0.34, 0.29, 0.22], c1: [0.26, 0.24, 0.21], tile: T_SMOKE,
        drag: 1.5, grav: -5.0, a0: 0.55,
        rot: R() * 6.283, rotV: (R() - 0.5) * 0.6,
      });
    }
    // ── 0.00s 탄도 흙줄기 ────────────────────────────────────────────
    // **가산 블렌딩은 어두운 것을 못 그린다.** 진짜 흙덩이는 하늘을 배경으로
    // 검은 실루엣이지만 가산은 더하기만 하므로 검정은 아무 일도 안 한다.
    // 그래서 덩어리가 아니라 **햇빛 받는 마른 먼지 꼬리**로 그린다 — 지면
    // 착탄은 대개 어두운 지형을 배경으로 보이니 옅은 따뜻한 줄이 잘 읽힌다.
    // 밝기는 선형 0.20 이다(노출 7.7배를 지나면 중간 밝기 위다). 파편의
    // 9.0 처럼 두면 흙이 불티가 된다.
    //
    // 속도 방향으로 늘린다(spark() 와 같은 수법). **수명을 탄도 비행시간에
    // 맞춰** 올라간 것이 되떨어져 지면에 닿을 즈음 꺼지게 했다 — 고정 수명을
    // 주면 빠른 것은 정점에서 사라지고 느린 것은 땅에 박힌 채 남는다.
    const G = 20;
    for (let i = 0; i < 16; i++) {
      const a = (i / 16) * 6.283 + R() * 0.4;
      const el = 0.5 + R() * 0.75;                     // 위로 치우친 원뿔
      const sp = 16 + R() * 26;
      const vy = Math.sin(el) * sp, hz = Math.cos(el) * sp;
      this._p('add', x, y + 0.5, z, Math.cos(a) * hz, vy, Math.sin(a) * hz, {
        life: Math.min(2.4, 2 * vy / G * (0.80 + R() * 0.25)),
        s0: 0.5, s1: 0.22,
        c0: [0.20, 0.16, 0.11], c1: [0.07, 0.058, 0.042], tile: T_SPARK,
        drag: 0.45, grav: -G, stretch: 0.10,
      });
    }
  }

  /** 수면 착탄 — 파문 링 1 · 물기둥 7 · 밑동 서지 5 · 물보라 20(가산). */
  _splash(x, y, z) {
    const R = Math.random;
    // 파문 — 수면(y=1)에 고정한다. 예전 그대로다.
    this._p('alpha', x, 1, z, 0, 0, 0, {
      life: 1.1, s0: 8, s1: 120, c0: [0.85, 0.95, 1.0], tile: T_RING, a0: 0.55,
    });
    // ── 0.00~0.12s 물기둥 ────────────────────────────────────────────
    // 예전 12개는 수평이 4~20m/s 라 기둥이 아니라 **공**이었다. 물은 갈 데가
    // 위밖에 없어 좁고 높게 선다 — 수평을 1/4 로 줄이고 위를 올렸다.
    for (let i = 0; i < 7; i++) {
      const a = R() * 6.283, sp = 1.5 + R() * 5;
      const vx = Math.cos(a) * sp, vy = 22 + R() * 24, vz = Math.sin(a) * sp;
      const d = (i / 7) * 0.12;
      this._p('alpha', x + vx * d, y + vy * d * 0.8, z + vz * d, vx, vy, vz, {
        delay: d,
        life: 1.3 + R() * 0.5 - d, s0: 3, s1: 20,
        c0: [0.86, 0.92, 0.98], c1: [0.62, 0.70, 0.78], tile: T_SMOKE,
        drag: 1.2, grav: -9.0, a0: 0.72,
        rot: R() * 6.283, rotV: (R() - 0.5) * 0.5,
      });
    }
    // ── 0.00~0.06s 밑동 서지 ─────────────────────────────────────────
    // 물기둥 밑에서 수면을 따라 퍼지는 흰 띠. 흙의 접지 고리와 같은 몫이다 —
    // 이것이 있어야 '수면에서' 터진 것으로 읽힌다.
    for (let i = 0; i < 5; i++) {
      const a = (i / 5) * 6.283 + R(), sp = 22 + R() * 24;
      const vx = Math.cos(a) * sp, vy = 0.6 + R() * 1.6, vz = Math.sin(a) * sp;
      const d = (i % 2) * 0.06;
      this._p('alpha', x + vx * d * 0.7, y + 0.8, z + vz * d * 0.7, vx, vy, vz, {
        delay: d,
        life: 1.4 + R() * 0.5 - d, s0: 4, s1: 28,
        c0: [0.80, 0.87, 0.94], c1: [0.58, 0.66, 0.74], tile: T_SMOKE,
        drag: 2.0, grav: -0.4, a0: 0.5,
        rot: R() * 6.283, rotV: (R() - 0.5) * 0.4,
      });
    }
    // ── 0.00s 물보라 ─────────────────────────────────────────────────
    // 흙줄기와 같은 탄도·같은 수명 규칙. 물방울은 햇빛을 되쏘므로 흙보다
    // 밝게(선형 0.34) 두고 푸르게 기울인다.
    const G = 20;
    for (let i = 0; i < 20; i++) {
      const a = (i / 20) * 6.283 + R() * 0.5;
      const el = 0.75 + R() * 0.65;                    // 흙보다 더 곧게 선다
      const sp = 18 + R() * 30;
      const vy = Math.sin(el) * sp, hz = Math.cos(el) * sp;
      this._p('add', x, y + 0.5, z, Math.cos(a) * hz, vy, Math.sin(a) * hz, {
        life: Math.min(2.4, 2 * vy / G * (0.80 + R() * 0.25)),
        s0: 0.42, s1: 0.18,
        c0: [0.28, 0.33, 0.40], c1: [0.10, 0.13, 0.17], tile: T_SPARK,
        drag: 0.5, grav: -G, stretch: 0.085,
      });
    }
  }
  ...
  build(P, eye, sortBack) {
    ...
    for (let k = 0; k < n; k++) {
      ...
      if (s[b + P_AGE] < 0) continue;          // 아직 안 태어났다 — 인스턴스를 안 낸다
